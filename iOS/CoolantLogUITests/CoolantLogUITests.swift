...
final class CoolantLogUITests: XCTestCase {
    ...
    func testFreeLimitTriggersPaywall() {
        ...
        for _ in 0..<9 {
            app.buttons["addButton"].tap()
            if app.staticTexts["Coolant Log Pro"].waitForExistence(timeout: 1) {
                break
            }
            let saveButton = app.buttons["saveButton"]
        ...
        XCTAssertTrue(app.staticTexts["Coolant Log Pro"].waitForExistence(timeout: 3))
    }
